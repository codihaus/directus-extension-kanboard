<template>
    <section>
        <header>
            <component :is="`display-${field?.meta?.display}`" v-bind="field?.meta?.display_options" :type="field?.type"
                :value="fieldValue" />
        </header>
        <main class="paginate-units">
            <paginate-unit v-for="page in pages" :key="page" :page="page" :collection-key="collectionKey" :field="field"
                :field-value="fieldValue" :layout-options="layoutOptions" :filter="filter" :search="search"
...
import PaginateUnit from "./paginate-unit.vue";

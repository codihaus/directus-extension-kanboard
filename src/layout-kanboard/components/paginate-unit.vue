<template>
    <paginate-unit-load-observer v-model:is-in-viewport="isInViewPort" :collection="fieldValue">
        <draggable class="paginate-unit" :list="items" item-key="id" :data-group="fieldValue" group="items"
            @end="handleDragEnd">
            <template #item="{ element }">
                <section class="card drag-handle" :data-item-id="element.id" @click="() => handleCardClick(element)">
                    <header>
                        <div v-if="layoutOptions?.iconTemplate" class="card-icon">
                            <render-template class="card-icon-inner" :template="layoutOptions.iconTemplate"
                                :collection="collectionKey" :item="element" />
                        </div>

                        <render-template v-if="layoutOptions?.headerTemplate" class="card-title"
                            :template="layoutOptions.headerTemplate" :collection="collectionKey" :item="element" />
                        <span v-else class="card-title muted">--</span>
                    </header>
                    <main v-if="layoutOptions?.cardContentTemplate">
                        <render-template :template="layoutOptions.cardContentTemplate" :collection="collectionKey"
                            :item="element" />
                    </main>
                </section>
            </template>
...
import { Field, Filter, Item, LogicalFilterAND } from "@directus/types";
import {
    computed,
    defineComponent,
    PropType,
...
import PaginateUnitLoadObserver from "./paginate-unit-load-observer.vue";
